  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        ...
        const res = await fetch(`${BASE_URL}/work-history`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        const data = await res.json();
        if (!res.ok) throw new Error(data.error || "Failed to fetch work history");
        setProjects(data.projects || []);
      } catch (err) {
        toast.error(err.message || "Unable to load work history");
        setProjects([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="min-h-screen bg-[url('/editbg.png')] bg-cover bg-center text-white pb-10">
      <div className="fixed w-screen top-0 z-50 bg-black/20 backdrop-blur-xl border-b border-white/10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center gap-6">
          ...
          <h1 className="text-3xl font-bold tracking-wide whitespace-nowrap">Work History</h1>
